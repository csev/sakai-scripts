...
if [ -z "$LOCAL_HOST_ACCESS" ]; then LOCAL_HOST_ACCESS="localhost"; fi
...
rm -f /tmp/sakai_mysql_setup.sql

cat << EOF > /tmp/sakai_mysql_setup.sql
...
echo
echo $MYSQL_COMMAND ...
cat /tmp/sakai_mysql_setup.sql
echo EOF

$MYSQL_COMMAND <  /tmp/sakai_mysql_setup.sql
